...
class ZlibSpiterSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in all_links:
            book_title = response.css("h3 a::text").extract()
            book_author = response.css(".color1:nth-child(1)::text").extract()
            year = response.css(".property_year .property_value").css("::text").extract()
            language = response.css(".text-capitalize::text").extract()
            file_type_size = response.css(".property__file .property_value").css("::text").extract()
            rate_interest = response.css(".book-rating-interest-score::text").extract()
            rate_quality = response.css(".book-rating-quality-score::text").extract()
            book_link = response.css("h3 a::attr(href)").extract()
